...
public class A05_0405 {
	...
	public static void main(String[] args) {
		...
		// 작은 데이터의 저장과 큰 데이터의 저장된 위치 -> stack의 메모리
		...
		// 5. 배열로 점수를 3개를 초기화하여, 반복문과 Scanner를 통해서 데이터를 입력되게 하고, 총계과 평균을 출력하세요.
			...
			System.out.println("총계 : " + tot + "\t평균 : " + (tot/point.length));
		
		// 6. 배열 3개에 각각 학생명, 국어, 영어를 할당하여, 반복문을 통해서 출력하세요.
			String ex06[] = {"Frank", "joe", "taylor"}; 
			int kor[] = {50, 60, 70};
			int eng[] = {70, 80, 90};			
		...
		// 7. 배열로 가위,바위,보라고 선언한 후, 두 친구가 index 0~2 사이에 임의로 호출하여, 임의의 가위/바위/보를 출력하게 하세요
			...
			System.out.println("친구1 : " + rpc[friend1] + ", 친구2 : " + rpc[friend2]);
		
		// 8. 웹서버와 웹클라이언트는 어떤 프로그램으로 실행되는지? network환경과 함께 기술해보세요.
			// 웹서버 : tomcat , 웹클라이언트 : 브라우저프로그램
			// 서버와 클라이언트 개념으로 웹서버가 데몬으로 구축되어 실행되고 
			// 클라이언트는 ip/port/경로의 자원명으로 접근하여 html코드를 랜더링(해석)함.
    ...
    <h2 align = "center">css 적용입니다</h2> // css - 확장된 웹페이지의 비주얼 부분을 설정
