...
public class A04_PreparedDao {
    ...
    public void updateEmp(Emp upt) {
        String sql = "UPDATE EMP02 SET ENAME = ?, job = ?, sal = ?, hiredate = to_date(?,'YYYY/MM/DD'), deptno = ? WHERE empno = ?";
        try {
            con = DB.con();
            con.setAutoCommit(false);
            pstmt = con.prepareStatement(sql);
	...
	public Code getCode(int no) {
		Code c = null;
		String sql = "SELECT * FROM CODE c WHERE NO = ?";
...
DELETE FROM CODE WHERE NO = ?			
			
	 * */
	
	public void  updataCode(Code upt) {
	...
	public void  deletCode(int no) {
	    String sql = "DELETE FROM CODE WHERE NO = ?";
	    try {
	        con = DB.con();
	        con.setAutoCommit(false);
	        pstmt = con.prepareStatement(sql);
	...
	public List<Job> getJob(String job_id) {
